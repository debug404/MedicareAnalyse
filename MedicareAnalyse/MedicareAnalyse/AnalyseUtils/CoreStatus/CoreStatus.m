...
/** 获取当前网络状态：枚举 */
+(NSString *)currentNetWorkStatus1{
    
    NSString *str = @"";
    CoreStatus *status = [CoreStatus sharedCoreStatus];
    str = [status statusWithRadioAccessTechnology1];
    return str;
}

/** 获取当前网络状态：字符串 */
+(NSString *)currentNetWorkStatusString{
    return [self currentNetWorkStatus1];
}
...
- (NSString *)statusWithRadioAccessTechnology1{
    ...
    if (status == CoreNetWorkStatusWifi) {
        type = @"1";
    }else if (status == CoreNetWorkStatus2G) {
        type = @"2";
    }else if (status == CoreNetWorkStatus3G) {
        type = @"3";
    }else if (status == CoreNetWorkStatus4G) {
        type = @"4";
    }else if (status == CoreNetWorkStatusUnkhow) {
        type = @"5";
    }else{
        type = @"0";
    }
...
-(NSArray *)coreNetworkStatusStringArray{
    ...
    if(_coreNetworkStatusStringArray == nil){
        ...
        _coreNetworkStatusStringArray = @[@"0",@"1",@"2",@"3",@"4",@"5"];
    }
